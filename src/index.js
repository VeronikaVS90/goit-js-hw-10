...
const onInput = debounce(evt => {
    ...
    fetchCountries(name).then(choiceCountry).catch(error => console.log(error));
}, DEBOUNCE_DELAY);
...
function renderCountryInfo(countries) {
  const markup = countries
    .map(country => {
      ...
      <img src="${country.flags.svg}" width="80" height="60" alt="flag of ${
...
function renderCountriesAll(countries) {
  const markup = countries
    .map(country => {
      ...
      <img src="${country.flags.svg}" width="80" height="60" alt="flag of ${country.name.official}">
